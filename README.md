...
- ✅ 데이터셋 설명, 라이선스, **받는 스크립트**를 공유한다 — 17종 전부 `fetch.py` 하나로 받는다
- ❌ **데이터 파일 자체는 커밋·재배포하지 않는다.** 스크립트가 각자 원출처(Zenodo·GitHub·AWS Open Data 등)에서 직접 받아온다 — 이 저장소는 "어디서 어떻게 받는지"만 알려준다
- ⚠️ 라이선스는 **원출처 기준**이다. 아래 표의 라이선스 표기를 반드시 확인하고, 상업적 이용이 걸린 경우 원문 약관을 직접 재확인할 것

## 카탈로그 (17종)

**받는 건 전부 `fetch.py` 하나로 한다** — `uv run python fetch.py <key>`. 받은 파일은 `data/` 아래에 쌓이고, 캐시 우선이라 다시 실행해도 이미 있는 건 건너뛴다.
...
> **17종 전부 스크립트로 받는다.** 브라우저로 직접 받아야 하는 것은 없다. 전부 받으면 약 **11.3 GB**다.
>
...
1 GB가 넘으면 받기 전에 용량을 보여주고 계속할지 묻는다(`-y`로 건너뛴다). 중간에 실패해도 다시 실행하면 **받은 것은 건너뛰고 실패한 것만** 다시 받는다.

### 스크립트 직접 쓰기

`fetch.py`는 진입점일 뿐이고, 실제 다운로드는 `scripts/` 아래 6개가 한다. 원출처 사정(Zenodo · figshare · AWS Open Data · GitHub raw)이 제각각이라 한 파일로 합치지 않았다. **세부 옵션은 스크립트에만 있다** — 아래 표의 예가 그것이다.
...
- **받는 법**: `uv run python fetch.py ids2018` (IP 열이 있는 1일치 약 4GB)
  - 10일치 전부(6.89GB)는 스크립트로: `uv run python scripts/fetch_ids2018.py --all`
...
- **받는 법**: `uv run python fetch.py cert` (r4.2 + 정답, 4.82 GB)
  - `http.csv`(2,843만 행·14.5GB)와 다른 릴리스는 스크립트로: `uv run python scripts/fetch_cert.py r4.2 --with-http`
...
- **받는 법**: `uv run python fetch.py enron` (443 MB — 스크립트는 받기만 하고 풀지 않는다. 풀면 2.6GB·파일 50만 개)
...
- **받는 법**: `uv run python fetch.py phishtank tranco` (rate limit 있음, 캐시 우선)
...
- 2026-09-17: **`fetch.py` 단일 진입점 신설** — 17종 전부 `uv run python fetch.py <key>`로 받는다. `--list`·`--group`·`--all`·`--dry-run`, 1GB 넘으면 확인을 묻는다. 실제 다운로드는 `scripts/` 6개가 그대로 하고 `fetch.py`는 key를 스크립트 인자로 넘기기만 한다 — 원출처 사정이 제각각이라 1,100줄을 한 파일로 합치지 않았고, 그래야 원본(`kbdc-poc`)과 동기화도 유지된다. 곁들여 `fetch_phishing_feeds.py`의 argparse 누락(`--help`를 줘도 도움말 대신 다운로드가 시작됐다)과 미사용 `EXTRA` 상수, 복사되면서 남은 `시나리오/` 경로 표기를 고쳤다.
